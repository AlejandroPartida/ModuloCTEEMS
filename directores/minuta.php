...
include("../connection.php"); 
    session_start();
    include("../verificar.php")?><!doctype html>
        ...
        if (isset($_POST['guardar']))
         {
           ...
           $programa=$_POST['pro'];
            ...
            $sqlUpload ="INSERT INTO dialogo_minuta (minuta,fk_programa) VALUES ('$ruta','$programa')";
 ...
 ?>
        <div class="content">
            ...
            <div class="col-md-4">
                ...
                <BR>
                <div class="card">
                    <a href="#" data-toggle="modal" data-target="#guardar">
                      ...
                      <div class="modal fade" id="guardar" tabindex="-1" role="dialog" aria-labelledby="smallModalLabel" aria-hidden="true">
                        ...
                        <?php $consulta="SELECT nom_prog, id from programas";
                         
                             $consulta2= $conn->query($consulta);
                             if ($consulta2->num_rows > 0) {
                                ?>
                        <select name="pro" class="form-control">
                                
                            <?php  while($fila = $consulta2->fetch_assoc()) {
                           echo" <option value='".$fila["id"]."' >".$fila["nom_prog"]."</option>";
                            } ?>
                        </select>
                    <?php }
                    else{
...
<!-- TABLA QUE MUESTRA EL COMITE -->
 <h5 class="modal-title" id="smallModalLabel"> <strong> ARCHIVOS MINUTA </strong></h5>
    <br>
                            <div class="card-body">
                                ...
                                <tbody>
<?php
                                            $sql1 = "SELECT dialogo_minuta.id as ide ,minuta,nom_prog FROM dialogo_minuta JOIN programas on dialogo_minuta.fk_programa=programas.id";
                                            ...
                                            while($row = $result1->fetch_assoc()) {
                                                $id = $row["ide"];
                                                $minuta = $row["minuta"];
                                                $fk_programa = $row["nom_prog"];

                                                
                                                echo  "<tbody>";
                                                echo     "<tr>";
                                                echo      "<td>" .utf8_decode($row["ide"])."</td>";
                                                echo      "<td>" .utf8_decode($row["minuta"]). "</td>";
                                                echo      "<td>" .utf8_decode($row["nom_prog"]). "</td>";
                                                 echo      "<td><form action='acuerdos.php' method='post'>
                                                 <input type='hidden' value=".utf8_decode($row["ide"])." name='ide'>
                                                 <input type='hidden' value=".utf8_decode($row["minuta"])." name='minuta'>
        
                                                 <input type='submit' class='btn' value='A G R E G A R'>
                                                </form></td>";
                                                echo     "</tr>";
